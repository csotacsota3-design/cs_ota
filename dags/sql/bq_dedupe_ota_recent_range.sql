...
CREATE TABLE IF NOT EXISTS `{{ params.GCP_PROJECT }}.{{ params.BIGQUERY_DS }}.{{ params.BIGQUERY_TABLE }}_deduped`
(
  request_id STRING,
  funnel_id STRING,
  session_id STRING,
...
DELETE FROM `{{ params.GCP_PROJECT }}.{{ params.BIGQUERY_DS }}.{{ params.BIGQUERY_TABLE }}_deduped`
WHERE source_day BETWEEN start_date AND end_date;

INSERT INTO `{{ params.GCP_PROJECT }}.{{ params.BIGQUERY_DS }}.{{ params.BIGQUERY_TABLE }}_deduped`
SELECT * EXCEPT(rn)
FROM (
  SELECT
    t.*,
    ROW_NUMBER() OVER (
      PARTITION BY request_id, funnel_id, session_id, page_name, timestamp
      ORDER BY ingested_at DESC, source_day DESC, source_file DESC
    ) AS rn
  FROM `{{ params.GCP_PROJECT }}.{{ params.BIGQUERY_DS }}.{{ params.BIGQUERY_TABLE }}` t
  WHERE t.source_day BETWEEN start_date AND end_date
)
WHERE rn = 1;
